...
try:
    ...
    # Importa a versão FINAL do terminal
    ...
    QMessageBox.critical(None, "Erro de Importação", f"Não foi possível importar um módulo necessário: {e}\nVerifique se todos os arquivos .py estão nos diretórios corretos (raiz e pasta \'ui\').\nDetalhes: {error_details}")
    sys.exit(1)
except Exception as e:
    error_details = traceback.format_exc()
    QMessageBox.critical(None, "Erro Inesperado", f"Ocorreu um erro inesperado durante a inicialização: {e}\nDetalhes: {error_details}")
    sys.exit(1)
...
class MainAppWindowFixed(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.status_bar.setStyleSheet("QStatusBar { background-color: #0a0a23; color: #ffffff; border-top: 1px solid #1e1e3f; }")
    ...
    def _run_code(self):
        ...
        temp_path = None
        try:
            self.status_bar.showMessage("Traduzindo código binário...")
            QApplication.processEvents()
            python_code = self.binary_interpreter.traduzir_binario(binary_code)
            self.status_bar.showMessage("Tradução concluída. Preparando execução...")
            QApplication.processEvents()

            terminal_instance = self._ensure_terminal()
            if not terminal_instance:
                self.status_bar.showMessage("Falha ao inicializar o terminal. Execução cancelada.")
                return
            ...
            self.status_bar.showMessage(f"Executando script temporário: {os.path.basename(temp_path)}...")
            terminal_instance.execute_python_script(temp_path)

        except Exception as e:
            error_details = traceback.format_exc()
            QMessageBox.critical(self, "Erro na Preparação", f"Erro durante a tradução ou preparação para execução:\n{str(e)}\nDetalhes: {error_details}")
            self.status_bar.showMessage("Falha na preparação para execução.")
        finally:
            # O arquivo temporário será limpo pelo terminal após a execução (se necessário)
            pass
