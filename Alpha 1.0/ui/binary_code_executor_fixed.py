...
class BinaryCodeExecutorFixed:
    ...
    def _execute_via_subprocess(self, python_code):
        ...
        if has_input:
            # Modifica o código para substituir chamadas input(...) por um valor padrão
            # Isso evita que o processo fique travado e previne erros de sintaxe
            # Usando regex para encontrar input(...) independentemente dos argumentos
            import re
            # Substitui input(...) por uma string padrão. Adicionamos aspas extras para que seja uma string literal no código Python.
            modified_code = re.sub(r"input\(.*?\)", "'Entrada Simulada'", python_code)
