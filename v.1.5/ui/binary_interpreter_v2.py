...
Este módulo corrige bugs e falhas nas verificações de comandos binários,
garantindo robustez na execução de códigos completos e interativos.
"""
...
class BinaryInterpreterV2:
    ...
    def __init__(self):
        ...
        # Adiciona mapeamentos para palavras-chave compostas
        self.text_to_binary.update({
            "print()": "10010010",
            "var": "01111011",
            "int": "10000000",
            "float": "10000001",
        ...
        # Comandos especiais que precisam de tratamento específico
        self.special_commands = {
            "10010010": "print()",  # print() sem argumentos
        }
    ...
    def converter_para_binario(self, text):
        ...
        # Processa palavras-chave compostas primeiro
        for keyword, binary in sorted(self.text_to_binary.items(), key=lambda x: len(x[0]), reverse=True):
            if len(keyword) > 1:  # Apenas palavras-chave compostas
                text = text.replace(keyword, f"<{binary}>")
        
        # Processa caracteres individuais
        for char in text:
            if char in self.text_to_binary:
                binary_code.append(self.text_to_binary[char])
            elif f"<{char}>" in text:
                # Já foi processado como palavra-chave
                continue
            else:
                # Caractere não reconhecido
                binary_code.append(f"<{char}>")
        
        # Substitui os marcadores de palavras-chave
        result = ' '.join(binary_code)
        for binary in self.text_to_binary.values():
            result = result.replace(f"<{binary}>", binary)
