...
class MainAppWindowFixed(QMainWindow):
    ...
    def _create_custom_menu_bar(self):
        ...
        self.run_in_terminal_button = self._create_action_button("▶ Executar no Terminal", "runInTerminalButton")
        self.run_in_terminal_button.clicked.connect(self._run_code_in_terminal)
        menu_layout.addWidget(self.run_in_terminal_button)
    ...
    def _run_code_in_terminal(self):
        """
        Traduz o código binário atual para Python, salva em arquivo temporário e executa no terminal integrado.
        """
        if self.tabs.count() == 0:
            QMessageBox.warning(self, "Aviso", "Nenhuma aba aberta para executar.")
            return
        ...
        try:
            ...
            # Envia o comando para executar o script no terminal
            python_executable = sys.executable
            cmd = f'"{python_executable}" -u "{temp_path}"\n'
            if hasattr(terminal_instance, 'process') and terminal_instance.process:
                terminal_instance.process.write(cmd.encode('utf-8'))
                self.status_bar.showMessage(f"Executando no terminal: {os.path.basename(temp_path)}")
            else:
                QMessageBox.warning(self, "Aviso", "Terminal não suporta execução automática. Execute manualmente:\n" + cmd)
        except Exception as e:
            QMessageBox.critical(self, "Erro", f"Erro ao executar no terminal:\n{str(e)}")
